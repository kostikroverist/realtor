...
// Динамічне імпортування компонентів react-leaflet (тільки на клієнті)
...
const Page = () => {
  ...
  const [customIcon, setCustomIcon] = useState<import("leaflet").DivIcon | null>(null);
  ...
  // Створення customIcon тільки на клієнті
  useEffect(() => {
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-grow">
        ...
        >
          ...
          {/* Динамічні маркери */}
          {flatProperties.length === 0 ? null : flatProperties.map((item, index) => {
            const lat = parseFloat(item.lat);
            const lng = parseFloat(item.lng);
            if (isNaN(lat) || isNaN(lng) || !customIcon) return null;
